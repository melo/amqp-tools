...
The L<Protocol::AMQP::Peer|Protocol::AMQP::Peer> is the where the AMQP
wire protocol is implemented. The frame parser, the channel manager, and
the method dispatcher.

Peer classes, L<Client|Protocol::AMQP::Client> and
L<Server|Protocol::AMQP::Server>, implement the logic from each side,
basically they handle methods and reply to them.

The L<Protocol::AMQP::Channel|Protocol::AMQP::Channel> is the object
that represents a open channel.


=head1 PROTOCOL VERSION CLASSES

Each protocol version is stored in a Protocol::AMQP::VMMMmmmrrr
namespace, where MMM, mmm and rrr are the major, minor and revision
numbers (0-padded).

These classes are generated by the C<amqp_api_generator.pl> from the
L<Parse::AMQP::ProtocolDefinitions|Parse::AMQP::ProtocolDefinitions>
distribution.
